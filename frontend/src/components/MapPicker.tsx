...
function MarkerDraggable({ setLatlng }: { setLatlng: Props['setLatlng'] }) {
  useMapEvents({
    click(e) {
...
export default function MapPicker({ latlng, setLatlng }: Props) {
  const center: LatLngExpression = latlng ? [latlng.lat, latlng.lng] : [20.5937, 78.9629];
  return (
    <div className="h-64 w-full rounded overflow-hidden border">
      <MapContainer center={center} zoom={latlng ? 15 : 5} style={{ height: '100%', width: '100%' }}>
        ...
        <MarkerDraggable setLatlng={setLatlng} />
      </MapContainer>
    </div>
  );
}